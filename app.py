...
=======================================
Streamlit frontend — reads from Supabase, renders Plotly charts.
Four pages: Market Overview / Stock Explorer / Technical Signals / Sector Analysis
"""
...
@st.cache_data(ttl=1800)   # Cache 30 min; avoids hammering Supabase
def load_data(days: int) -> pd.DataFrame:
    ...
    result = (
        ...
        .select("ticker,date,open,high,low,close,volume,"
                "daily_return,ma20,ma60,rsi,volatility,name,sector")
        .gte("date", since)
        .order("date")
        .execute()
    )
    ...
    if not df.empty:
        ...
        num_cols = ["open", "high", "low", "close",
                    "daily_return", "ma20", "ma60", "rsi", "volatility"]
...
if df.empty:
    st.error("⚠️ No data in database. Run `python pipeline.py` first.")
    st.stop()

# Latest row per ticker (most recent session)
...
if page == "📊 Market Overview":
    ...
    fig_map.update_coloraxes(colorbar_title="Return %")
    ...
    # Cumulative return chart – top 5 performers over the period
    st.subheader(f"Cumulative returns – top 5 stocks (last {days} days)")
    top5_tkrs = (df.groupby("ticker")["daily_return"]
    ...
    fig_cum = px.line(
        df5, x="date", y="cum_ret", color="name",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    name_map = (df[["ticker", "name"]].drop_duplicates()
                .set_index("ticker")["name"].to_dict())
    ...
    if s.empty:
        st.warning("No data for the selected stock.")
        st.stop()
    ...
    c1.metric("Close (TWD)", f"{row['close']:,.1f}",
    ...
    # Candlestick + MA overlays
    fig_cs = go.Figure()
    fig_cs.add_trace(go.Candlestick(
        x=s["date"], open=s["open"], high=s["high"],
    ...
    fig_cs.add_trace(go.Scatter(
        x=s["date"], y=s["ma20"], name="MA20",
        line=dict(color="#f39c12", width=1.5)))
    fig_cs.add_trace(go.Scatter(
        x=s["date"], y=s["ma60"], name="MA60",
    ...
    with col2:
        fig_rsi = go.Figure()
        fig_rsi.add_hrect(y0=70, y1=100, fillcolor="red",
                           opacity=0.06, line_width=0)
        fig_rsi.add_hrect(y0=0,  y1=30,  fillcolor="green",
                           opacity=0.06, line_width=0)
        fig_rsi.add_trace(go.Scatter(
            x=s["date"], y=s["rsi"], name="RSI",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        if not oversold.empty:
            st.dataframe(oversold.rename(
                columns={"name": "Company", "close": "Price (TWD)"}),
        ...
        if not overbought.empty:
            st.dataframe(overbought.rename(
                columns={"name": "Company", "close": "Price (TWD)"}),
                hide_index=True, use_container_width=True)
        else:
    ...
    with col2:
        ...
        fig_rr = px.scatter(
            latest, x="volatility", y="daily_return",
            color="sector", hover_name="name",
            hover_data={"ticker": True, "close": ":,.0f"},
            labels={"volatility":    "Ann. volatility (%)",
                    "daily_return":  "Daily return (%)"},
        )
        fig_rr.add_hline(y=0, line_dash="dash",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    sector.columns = ["Sector", "Avg return (%)", "Avg RSI",
                      "Avg volatility (%)", "# stocks", "Total volume"]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    **相關係數**衡量兩支股票報酬率的同步程度：
    ...
    for i, j in itertools.combinations(range(len(cols_list)), 2):
        pairs.append({'股票 A': cols_list[i], '股票 B': cols_list[j],
                      '相關係數': corr.iloc[i, j]})
    pairs_df = pd.DataFrame(pairs).sort_values('相關係數', ascending=False)

    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**🔴 最高正相關（風險集中）**")
        st.dataframe(pairs_df.head(5).reset_index(drop=True),
                     hide_index=True, use_container_width=True)
    with c2:
        st.markdown("**🔵 最低相關（最佳分散效果）**")
